...
import com.birthdaynote.library.event.DefEvent;
import com.birthdaynote.library.event.EventData;
...
public class RxBus {
    ...
    public Observable<DefEvent> toObservableUiThread(final String eventType, final String tag) {
        return toObservable(DefEvent.class).compose(RxUtils.schedulersTransformer()).filter(new Predicate<DefEvent>() {
            @Override
            public boolean test(DefEvent defEvent) throws Exception {
                if (defEvent.getType().equals(eventType) && defEvent.getTag().equals(tag))
                    return true;
